...
%---------------------------------------------------------------------------
  ...
  m=0.23223;  %23223/100000  %0.3; %migration rate
  ...
  R_k={};  %Composicion de species @ each Site and each Generation k  
    ...
    for ii = 1:length(As); % i refersw to values of amplitude
     for jj = 1:length(GPTs); % j refers to values of frequency
        ...
        fnam = sprintf('ASymD_A%0.4f_GPT%04d',As(1,ii),GPTs(1,jj));
        
          %start loop of replicates
          for ri = 1:MaxRep;   
            %tic
            fnam_positions=[fnam '_r' num2str(ri) '_pos.dat'];
            fnam_Compositions=[fnam '_r' num2str(ri) '_comp.dat'];
            fnam_migrations=[fnam '_r' num2str(ri) '_migr.dat'];
            %Create the random geometric graph 
            n = unifrnd(0,L,S,2); %positions of sites            
            save(fnam_positions,'n'); %Save Site positions for realization ri
            ...
            for k = 1:MaxGenerations,  %Generations  
              ...
              for i = 1:S-1,
                for j = i+1:S,
                  ...
                  d(i,j) = sqrt(dx2 + dy2);
                  %Di(i,j) = 1/d(i,j);
                  Sd(i,j) = 1/d(i,j);  %Score because of distance (symmetric case)
                  
                  %Di(i,j) = 1/(sqrt(dx2 + dy2));
              ...
              St_ij=Sd_ij; %Total Score is due to distance only SYMMETRIC ON!!!
...
%                b = repmat(a,1,S);
%                DIa = bsxfun(@rdivide,DI,b);
%                Dc = cumsum(DIa(1:S,1:S),2);
            ...
            %fnam = sprintf('Symmetry_A%0.4f_GPT%04d',As(1,ii),GPTs(1,jj));
            
            save(fnam_Compositions,'R_t');  %Save Compositions time series for realization ri

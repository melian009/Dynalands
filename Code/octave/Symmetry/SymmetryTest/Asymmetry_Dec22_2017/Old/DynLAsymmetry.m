        ...
        Gamma2Acum=zeros(1,MaxGenerations);
          ...
          for ri = 1:MaxRep;       
            ...
            for k = 1:MaxGenerations,  %Generations       
              ...
              %ASYMMETRIC MIGRATION ==> Dc means mij == mji for all pairs
              DI=Di+Di';
              D1=D+D';
              DI=DI.*D1;  %<========ALEX
              Dc=cumsum(DI,2);
        
              %Ddemographic events of the generation happen according to their probability
          ...
          fnam = sprintf('ExactSymmetry%d %d.txt',As(1,ii),GPTs(1,jj));
          fid = fopen(fnam,'w');
          fprintf(fid,'%f %f %f %f %f \n',ri,countgen,gamma,alphaM,alphaSD);fclose(fid);  
            
          %save gamma for all the generations per replicate 

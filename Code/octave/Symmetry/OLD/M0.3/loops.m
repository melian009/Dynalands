%Distribution of loops of size 1, 2, 3... in a graph
...
%---------------------------------------------------------------------------
  ...
  MaxGenerations = 2000; %number of generations per replicates
        ...
        Gamma2Acum=zeros(1,MaxGenerations);
          ...
          for ri = 1:MaxRep;   
            ...
            for k = 1:MaxGenerations,  %Generations  
              ...
              %St_ij=K*exp(-d_ij/L); %Total Score is due to distance only SYMMETRIC ONE!!!  ALEX: Symmetric!!!!!
              St_ij=P_min^(d_ij/(sqrt(2*L)));
...
%                Dc = cumsum(DIa(1:S,1:S),2);
              ...
              for t = 1:J*S,  %MonteCarlo Time
                ...
                if ep < m,  %Migration
                    ...
                    MigrantHabProb = unifrnd(0,1);%unifrnd(0,max(P_ij(:,KillHab)));    ALEX: Symmetric!!!!

...
%LAT classes 
LATclas = 60;%latitude classes :: final run 100 classes
...
Timebin = unique(c(:,1));
 for k2 = 1:length(Timebin); 
 k2
 ##Account for time when calculating the gamma pool
...
%plot(alpha,gamma,'.','Markersize',28,'Color',[red green blue])%Works
    ...
    cell2csv('Ttime60LATclassPhyto.csv',T);
end
